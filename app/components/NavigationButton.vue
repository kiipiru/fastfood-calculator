...
<template>
    <button @click="emit('navigation-button-clicked')" class="size-12 p-2 rounded-full relative">
        <span v-if="amount" class="text-white px-2 py-1 rounded-full absolute -top-1 -right-1 size-6 text-xs" :class="spanBgClass">{{ amount }}</span>
