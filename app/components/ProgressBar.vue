...
const colorClasses = computed(() => {
    ...
    else {return {text: 'text-red-400', bar: 'bg-red-400'}}
})
...
<template>
    <div class="flex justify-between">
        <span>{{ title }}</span>
        <span :class="colorClasses.text" class="font-semibold">{{ value }} / {{ max }} {{ unit }}</span>
    </div>
    <div class="bg-gray-200 w-full rounded-full h-2 overflow-hidden">
        <div class="h-full rounded-xl" :class="colorClasses.bar" :style="{ width: progressBarPercent + '%' }"></div>
    </div>
</template>